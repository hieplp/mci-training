...
| `SumController` | `GET /` — echoes the operands, adds the `SumResult` (or the validation error) to the model. |
| `SumService` / `SumServiceImpl` | Web-layer facade delegating to `MyBigNumber.sumWithSteps`. |

Package: `dev.hieplp.mci.web`

UI: single `index.html` page (Thymeleaf) + hand-rolled `static/css/app.css`,
showing the column-addition steps returned by `mci-core`.
